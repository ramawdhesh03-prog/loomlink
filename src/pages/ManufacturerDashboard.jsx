...
export default function ManufacturerDashboard() {
  ...
  const handleStatusUpdate = async (orderId, newStatus) => {
    ...
    if (!error) {
      ...
      showPopup(`Order status updated: ${STATUS_CONFIG[newStatus].label} ✅`)
    } else {
  ...
  const handleCatalogUpload = async (e) => {
    ...
    if (invalidFiles.length) { showPopup('Sirf JPG, PNG, WEBP ya PDF upload kar sakte hain', 'error'); return }

    const maxSize = 10 * 1024 * 1024
    const tooBig = files.filter(f => f.size > maxSize)
    if (tooBig.length) { showPopup('File size 10MB se kam honi chahiye', 'error'); return }
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const { error } = await supabase.storage.from('catalogs').upload(filePath, file, { cacheControl: '3600', upsert: false })
      if (error) showPopup(`${file.name} upload failed: ${error.message}`, 'error')
    }
  ...
  const handleDeleteCatalog = async (fileName) => {
    ...
    const filePath = `manufacturer_${profile.id}/${fileName}`
    const { error } = await supabase.storage.from('catalogs').remove([filePath])
    if (!error) { showPopup('File delete ho gayi'); fetchCatalogs(profile.id) }
    else showPopup('Delete failed: ' + error.message, 'error')
  }

  const handleSave = async () => {
    const { error } = await supabase.from('manufacturers').update({
      name: form.name, business_name: form.business_name, city: form.city,
      whatsapp: form.whatsapp, moq: form.moq, price_range: form.price_range,
      saree_types: form.saree_types,
    }).eq('id', profile.id)

    if (!error) { setProfile(form); setEditing(false); showPopup('Profile save ho gaya! ✅') }
    else showPopup('Save failed: ' + error.message, 'error')
  }
  ...
  const isImage = (fileName) => ['jpg', 'jpeg', 'png', 'webp'].includes(fileName.split('.').pop().toLowerCase())
  ...
  if (loading) return <div style={{ textAlign: 'center', padding: '100px', color: '#1B3A6B', fontSize: '1.2rem' }}>Loading...</div>
  ...
  const tabStyle = (tab) => ({
    ...
    background: activeTab === tab ? '#8B1A1A' : 'transparent',
    color: activeTab === tab ? 'white' : '#8B1A1A',
    border: '2px solid #8B1A1A', transition: 'all 0.2s'
  })
    ...
    <main style={{ padding: '40px 24px', maxWidth: '800px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '24px', flexWrap: 'wrap' }}>
        ...
        <button style={tabStyle('orders')} onClick={() => setActiveTab('orders')}>
          📦 Orders {pendingCount > 0 && <span style={{
      ...
      {/* PROFILE TAB */}
      {activeTab === 'profile' && (
        <div style={{ background: '#fff', borderRadius: '16px', padding: '32px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)', marginBottom: '24px' }}>
          <h2 style={{ color: '#1B3A6B', fontFamily: "'Playfair Display', serif", marginBottom: '24px' }}>📋 Mera Profile</h2>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
            {[
              ...
              <div key={field.key}>
                <label style={{ display: 'block', fontWeight: 600, fontSize: '0.85rem', color: '#888', marginBottom: '6px' }}>{field.label}</label>
                {editing ? (
          ...
          <div style={{ marginTop: '24px' }}>
            <label style={{ display: 'block', fontWeight: 600, fontSize: '0.85rem', color: '#888', marginBottom: '10px' }}>Saree Types</label>
            {editing ? (
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {(profile?.saree_types || []).map(type => (
                  <span key={type} style={{ padding: '6px 16px', borderRadius: '20px', background: '#8B1A1A', color: 'white', fontSize: '0.85rem' }}>{type}</span>
                ))}
              </div>
            )}
          </div>
      ...
      {activeTab === 'orders' && (
        <div style={{ background: '#fff', borderRadius: '16px', padding: '32px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
          <h2 style={{ color: '#1B3A6B', fontFamily: "'Playfair Display', serif", marginBottom: '24px' }}>📦 Incoming Orders</h2>

          {orders.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '60px', color: '#aaa', fontFamily: "'Mukta', sans-serif" }}>
              Abhi koi order nahi aaya hai।
            </div>
          ) : orders.map(order => (
            <div key={order.id} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px' }}>
                <div>
                  <div style={{ fontWeight: 700, fontSize: '1rem', color: '#1B3A6B', fontFamily: "'Playfair Display', serif" }}>
                    {order.wholesaler_business || 'Wholesaler'}
                  </div>
              ...
              {order.wholesaler_whatsapp && (
                <a href={`https://wa.me/91${order.wholesaler_whatsapp}?text=Hi%2C%20your%20order%20on%20LoomLink%20is%20being%20processed`}
              ...
              {order.status !== 'delivered' && order.status !== 'cancelled' && (
                <div style={{ display: 'flex', gap: '8px', marginTop: '14px', flexWrap: 'wrap' }}>
                  {order.status === 'pending' && (
                    <>
                      <button onClick={() => handleStatusUpdate(order.id, 'confirmed')} style={actionBtn('#1B3A6B')}>✅ Confirm</button>
                      <button onClick={() => handleStatusUpdate(order.id, 'cancelled')} style={actionBtn('#D32F2F')}>❌ Cancel</button>
                    </>
                  )}
                  {order.status === 'confirmed' && (
                    <button onClick={() => handleStatusUpdate(order.id, 'shipped')} style={actionBtn('#7B1FA2')}>🚚 Mark Shipped</button>
                  )}
                  {order.status === 'shipped' && (
                    <button onClick={() => handleStatusUpdate(order.id, 'delivered')} style={actionBtn('#2D7A4A')}>📦 Mark Delivered</button>
                  )}
                </div>
              )}
            </div>
      ...
      {activeTab === 'catalog' && (
        <div style={{ background: '#fff', borderRadius: '16px', padding: '32px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
          <h2 style={{ color: '#1B3A6B', fontFamily: "'Playfair Display', serif", marginBottom: '8px' }}>📁 Mera Catalog</h2>
          <p style={{ color: '#888', fontSize: '0.9rem', marginBottom: '24px', fontFamily: "'Mukta', sans-serif" }}>
            Apne saree catalog ki images ya PDF yahan upload karein।
          </p>
          <label style={{
            display: 'block', border: '2px dashed #F5A623', borderRadius: '16px', padding: '36px',
            textAlign: 'center', cursor: uploading ? 'not-allowed' : 'pointer',
            background: uploading ? '#fffbf0' : '#fffdf5', transition: 'all 0.2s', marginBottom: '24px'
          }}>
            <input type="file" multiple accept=".jpg,.jpeg,.png,.webp,.pdf" onChange={handleCatalogUpload} disabled={uploading} style={{ display: 'none' }} />
            {uploading ? (
              <div>
                <div style={{ fontSize: '2rem', marginBottom: '8px' }}>⏳</div>
                <div style={{ color: '#E8821A', fontWeight: 600, fontFamily: "'Mukta', sans-serif" }}>{uploadProgress}</div>
                <div style={{ color: '#888', fontSize: '0.85rem', marginTop: '4px' }}>Please wait...</div>
              </div>
            ) : (
              <div>
                <div style={{ fontSize: '2.5rem', marginBottom: '8px' }}>📤</div>
                <div style={{ color: '#1B3A6B', fontWeight: 700, fontSize: '1.1rem', fontFamily: "'Mukta', sans-serif" }}>Click karke files upload karein</div>
                <div style={{ color: '#888', fontSize: '0.85rem', marginTop: '6px' }}>JPG, PNG, WEBP ya PDF • Max 10MB • Multiple files allowed</div>
              </div>
            )}
          </label>

          {catalogs.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px', color: '#aaa', fontFamily: "'Mukta', sans-serif", fontSize: '0.95rem' }}>
              Abhi koi catalog upload nahi hua hai। Upar se upload karein! 👆
            </div>
          ) : (
            <div>
              <h3 style={{ color: '#1B3A6B', fontSize: '1rem', marginBottom: '16px', fontFamily: "'Mukta', sans-serif" }}>Uploaded Files ({catalogs.length})</h3>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '16px' }}>
                {catalogs.map(file => (
                  <div key={file.name} style={{ border: '1.5px solid #f0f0f0', borderRadius: '12px', overflow: 'hidden', background: '#fafafa', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
                    {isImage(file.name) ? (
                      <a href={file.url} target="_blank" rel="noreferrer">
                        <img src={file.url} alt={file.name} style={{ width: '100%', height: '150px', objectFit: 'cover', display: 'block' }} />
                      </a>
                    ) : (
                      <a href={file.url} target="_blank" rel="noreferrer" style={{ textDecoration: 'none' }}>
                        <div style={{ height: '150px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #fff5f5, #ffe8e8)' }}>
                    ...
                    <div style={{ padding: '12px' }}>
                      <div style={{ fontSize: '0.8rem', color: '#333', fontWeight: 600, fontFamily: "'Mukta', sans-serif", whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        {getFileIcon(file.name)} {file.name.split('_').slice(2).join('_') || file.name}
                      </div>
                      <div style={{ fontSize: '0.75rem', color: '#aaa', marginTop: '4px' }}>{formatFileSize(file.metadata?.size)}</div>
                      <div style={{ display: 'flex', gap: '8px', marginTop: '10px' }}>
                        <a href={file.url} target="_blank" rel="noreferrer" style={{ flex: 1, padding: '6px', textAlign: 'center', background: '#1B3A6B', color: 'white', borderRadius: '6px', fontSize: '0.78rem', textDecoration: 'none', fontFamily: "'Mukta', sans-serif" }}>👁️ View</a>
                        <button onClick={() => handleDeleteCatalog(file.name)} style={{ flex: 1, padding: '6px', background: '#fff0f0', color: '#D32F2F', border: '1px solid #ffcdd2', borderRadius: '6px', cursor: 'pointer', fontSize: '0.78rem', fontFamily: "'Mukta', sans-serif" }}>🗑️ Delete</button>
                      </div>
                    </div>
                  </div>
                ))}
...
const actionBtn = (bg) => ({
  padding: '8px 18px', background: bg, color: 'white',
