...
export default function AdminDashboard() {
  ...
  if (loading) return (
    <div style={{ textAlign: 'center', padding: '100px', color: '#1B3A6B' }}>
      Loading...
    </div>
  )
  ...
  return (
    <main style={{ padding: '40px 24px', maxWidth: '1100px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '24px' }}>
        <button style={tabStyle('manufacturers')} onClick={() => setActiveTab('manufacturers')}>
          🏭 Manufacturers ({manufacturers.length})
        </button>
        <button style={tabStyle('wholesalers')} onClick={() => setActiveTab('wholesalers')}>
          🛍️ Wholesalers ({wholesalers.length})
        </button>
      </div>
              ...
              <thead>
                <tr style={{ background: '#8B1A1A', color: 'white' }}>
                  {['Name', 'Business', 'City', 'Phone', 'WhatsApp', 'MOQ', 'Price Range', 'Registered'].map(h => (
                    <th key={h} style={{ padding: '14px 16px', textAlign: 'left', fontSize: '0.85rem', fontFamily: "'Mukta', sans-serif", whiteSpace: 'nowrap' }}>
                      {h}
                    </th>
                  ))}
                </tr>
                ...
                {manufacturers.length === 0 ? (
                  ...
                  <tr key={m.id} style={{ background: i % 2 === 0 ? '#fff' : '#fafafa', borderBottom: '1px solid #f0f0f0' }}>
                    ...
                    <td style={{ padding: '12px 16px', fontSize: '0.9rem' }}>
                      {m.whatsapp && (
                        <a href={`https://wa.me/91${m.whatsapp}`} target="_blank" rel="noopener noreferrer"
                    ...
                    <td style={{ padding: '12px 16px', fontSize: '0.9rem', color: '#888', whiteSpace: 'nowrap' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {wholesalers.length === 0 ? (
                  <tr>
                    <td colSpan={8} style={{ textAlign: 'center', padding: '40px', color: '#888' }}>
                      Abhi koi wholesaler registered nahi hai
                    </td>
                  </tr>
                ) : wholesalers.map((w, i) => (
                  <tr key={w.id} style={{ background: i % 2 === 0 ? '#fff' : '#fafafa', borderBottom: '1px solid #f0f0f0' }}>
                    ...
                    <td style={{ padding: '12px 16px', fontSize: '0.9rem' }}>
                      {w.whatsapp && (
                        <a href={`https://wa.me/91${w.whatsapp}`} target="_blank" rel="noopener noreferrer"
                          style={{ color: '#25D366', fontWeight: 600, textDecoration: 'none' }}>
                          {w.whatsapp}
                        </a>
                      )}
